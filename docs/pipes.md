...
# filter |> filter => single filter (Credo.Check.Refactor.FilterFilter)
# Two captures are joined with `&&`:
a |> Enum.filter(&(&1.x > 0)) |> Enum.filter(&(&1.y < 0)) |> ...
# Styled:
a |> Enum.filter(&(&1.x > 0 && &1.y < 0)) |> ...
# When an anonymous function is involved, the predicates become an `if`:
a |> Enum.filter(&(&1.x > 0)) |> Enum.filter(fn item -> expensive?(item) end) |> ...
# Styled:
a |> Enum.filter(fn item -> if item.x > 0, do: expensive?(item), else: false end) |> ...
